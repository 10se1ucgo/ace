...
SpadeTool::SpadeTool(ace::world::DrawPlayer& ply) : Tool(ply), mdl(ply.scene.models.get("spade.kv6"), 0.05f) {
...
bool SpadeTool::on_primary() {
    ...
    if (f != Face::INVALID && glm::all(glm::lessThan(glm::abs(this->ply.p - glm::vec3(hit)), glm::vec3(3.5f)))) {
...
bool SpadeTool::on_secondary() {
    glm::ivec3 hit;
    Face f = this->ply.scene.map.hitscan(this->ply.e, this->ply.f, &hit);
    if (f != Face::INVALID && this->ply.local_player && glm::distance2(this->ply.p, glm::vec3(hit)) < 3 * 3) {
...
bool GrenadeTool::on_primary() {
    ...
    this->ply.scene.create_object<ace::world::Grenade>(this->ply.p, this->ply.f + this->ply.v, this->fuse);
    this->primary_ammo--;
    return true;
}
...
bool Weapon::on_primary() {
    ...
    for(int i = 0; i < this->pellets(); i++) {
        ...
        dir += glm::vec3{ dist(eng), dist(eng), dist(eng) };
        this->ply.scene.create_object<ace::world::Tracer>(this->tracer(), this->ply.e + dir * 4.f, dir);
        ...
        for(auto &kv : this->ply.scene.players) {
            ...
            this->ply.scene.create_object<ace::world::DebrisGroup>(h, glm::vec3{ 127, 0, 0 }, 0.25f, 4);
            if(this->ply.local_player) {
                net::HitPacket hp;
