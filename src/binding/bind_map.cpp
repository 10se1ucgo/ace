...
void bind_map(py::module &a) {
    ...
    m.def("get_pos", &ace::get_pos);
    m.def("is_valid_pos", py::overload_cast<int, int, int>(&ace::is_valid_pos));
    m.def("is_valid_pos", py::overload_cast<size_t>(&ace::is_valid_pos));
    m.def("is_valid_pos", py::overload_cast<glm::ivec3>(&ace::is_valid_pos));
    m.def("jit_color", &ace::jit_color);

    py::enum_<ace::Face>(m, "Face")
